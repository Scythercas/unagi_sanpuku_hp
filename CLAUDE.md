...
方針は「色は写真が持ち、地と文字は無彩色に徹する」。全面を鉄紺で通し、縦スクロールの一本道で上から読ませる。トークンは `src/styles/global.css` の `@theme` に集約し、コンポーネントは必ずトークン名（`bg-tetsu` / `text-paper` 等）で参照する。**生の hex やデフォルトの Tailwind パレット（`stone-*` / `amber-*` 等）を直接書かない。**

| トークン | 値 | 用途 |
|---|---|---|
| `tetsu` | `#17184b` | 鉄紺。全面の地 |
| `tetsu-deep` | `#101132` | 一段沈めた地。セクションの区切りとフッター |
| `tetsu-line` | `#2e2f63` | 地の上の罫線 |
| `paper` | `#f7f7fa` | 主文字 |
| `paper-dim` | `#a3a4bd` | ラベル・キャプション・副次情報 |
| `dummy` | `#35366b` | 撮影待ちのダミー面 |

意匠上の決めごと：

- **差し色を置かない。** 色みは写真だけが持つ。強調は文字サイズと余白で作る。
- **角丸を使わない**（`rounded-*` を書かない）。写真も影なし・直角。
- **文字を写真に重ねない。** 例外はヒーローの店名のみで、その場合も地色のグラデーションで確実に沈めてから載せる。
- **タブを使わない。** 縦スクロール一本道。順番は ヒーロー → 店主の言葉 → お品書き → 佇まい → 店舗案内 → 求人 → お知らせ → フッター。
- ヒーローは画面の82%の高さに留め、下端に次を覗かせてスクロールできることを示す。
- お品書きは代表数品を写真付きで大きく、残りは文字だけの一覧に分ける（`splitFeaturedMenu()`）。
- 佇まいは横スクロール（`.hscroll`）。**スクロールバーは消さない**（横に送れることに気づけなくなるため）。
- 価格・電話番号・日付には `tabular-nums` を付けて桁を揃える。
- 見出しはカタカナ（メニュー／ギャラリー）ではなく**和語**（お品書き／佇まい／店舗案内／求人）を使う。

### 撮影待ちのダミー面

写真がまだ無い箇所は `Photo.astro` が自動でダミー面に切り替える。呼び出し側は写真の有無を分岐しない。

**ダミーには必ず `shootNote` で「何を撮るか」を書く。** 無地で埋めると本番写真と見分けがつかず、何が足りないのかも伝わらない。文面はそのまま撮影依頼として使えるよう具体的に書く（品目によって撮り方が違うため、`Menu.astro` の `shootNoteFor()` はカテゴリで文面を変えている）。

### 店章（favicon）— 未調整

`public/favicon.*` は老舗案（`develop`）で作った**弁柄地に筆文字の「さ」**の落款のまま。このデザインの配色（鉄紺・無彩色）とは合っていない。どちらの案を採用するか決まっていないため、意図的に手を付けていない。**この案を採用するなら、鉄紺地のゴシックで作り直すこと。**

### 見出しフォントのサブセット

本文はシステムゴシックスタック（`--font-gothic`）で追加ダウンロードなし。見出しと店名のみ、Noto Sans JP を**表示に使う文字だけのサブセット**（`public/fonts/noto-sans-jp-{400,700}-subset.woff2`、各20KB前後）で自己ホストする。

**見出しの文言を増やしたら再生成が必要**（未収録の文字はシステムゴシックにフォールバックし、字面が混ざる）。手順：

1. セクション見出し、`MENU_CATEGORIES`、`GALLERY_TYPES`、ヒーローの小見出し、下層ページ見出しに含まれる文字を集める。
2. Google Fonts の `css2?family=Noto+Sans+JP:wght@<400|700>&text=<文字列>` を取得し、`url(...)` の woff2 を上記パスへ保存。
...
1. ヒーロー：全画面写真1枚（画面の82%高）＋店名。写真は `shop.heroImage` を正とし、未設定ならギャラリーの「調理」→先頭 の順に代替する。**電話 CTA ボタンはヒーローには置かない**。
2. 店主の言葉：`shop.catchcopy` を見出し、`shop.ownerMessage` を本文、`shop.ownerImage` を人物写真として組む。両方空ならセクションごと非表示。
3. お品書き：`splitFeaturedMenu()` で代表数品と残りに分ける。代表は写真付きで左右交互に大きく、残りはカテゴリ別の文字一覧。価格は数値を「1,800円」等に整形。
4. 佇まい：`gallery` を種別順（外観→内装→調理→料理）に並べ、横スクロールで見せる。足りない枚数はダミー枠で埋める。
5. 店舗案内：住所・営業時間・定休日・駐車場・席数、Google マップ iframe、`tel:` リンク。
6. 求人：`recruit.isOpen` が true のときのみセクションごと出す。
7. お知らせ（`news`。臨時休業等。空なら非表示）
8. フッター（`tetsu-deep` の地。店名・住所・営業時間・**電話**・Instagram・コピーライト・プライバシーポリシー）
...
電話番号は `tel:` リンク（`TelLink.astro`）にする。設置箇所は**店舗案内セクション**と**フッター**の2か所。ヒーローには置かない（全画面写真と店名の見せ場を妨げるため）。フッターは全ページ共通なので、`/privacy` や 404 からも電話に到達できる。
