...
`{{DOMAIN}}`（既定 sanpuku-unagi.com・要確定）, `{{GA4_MEASUREMENT_ID}}`, 店舗の住所・電話・営業時間等（`docs/CONTENT-CHECKLIST.md` 参照）。

確定済み: `{{GITHUB_USERNAME}}` = `Scythercas`, `{{REPO_NAME}}` = `unagi_sanpuku_hp`（<https://github.com/Scythercas/unagi_sanpuku_hp>）, `{{MICROCMS_SERVICE_DOMAIN}}` = `unagisanpuku`（`unagisanpuku.microcms.io`）。
